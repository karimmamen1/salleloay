...
export function AuditList() {
  ...
  useEffect(() => onSnapshot(query(collection(db, "auditLogs"), orderBy("timestamp", "desc"), limit(300)), (snapshot) => setLogs(snapshot.docs.map((item) => ({ id: item.id, ...item.data() }) as AuditLog))), []);
  const filtered = useMemo(() => logs.filter((log) => {
    const logDate = log.timestamp?.toDate ? new Intl.DateTimeFormat("en-CA", { timeZone: "Africa/Algiers", year: "numeric", month: "2-digit", day: "2-digit" }).format(log.timestamp.toDate()) : "";
