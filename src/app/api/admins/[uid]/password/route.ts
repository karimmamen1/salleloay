import { z } from "zod";
import { AdminApiError, auditRecord, createWrite, firestoreCommit, firestoreGet, jsonError, randomDocumentPath, requireSuperAdmin, updateAuthUser, withAdminContext } from "@/lib/firebase/admin-server";
...
export async function POST(request: Request, context: { params: Promise<{ uid: string }> }) {
  return withAdminContext(request, async () => {
    try {
      ...
      await updateAuthUser(uid, { password: parsed.data.password });
